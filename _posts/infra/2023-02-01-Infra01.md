---
title:  "GitActions CI/CD(1) - git submodule 포함시키키" 
excerpt: "GitActions로 인프라를 구축해보자."
...
GitLab 배포 스크립트는 간단히 작성할줄만 알고, GitActions 배포 스크립트는 작성하는데에 두려움이 있었다.

또한, Github에서 Organization을 생성해서 진행하던 중이라 어쩔수없이 GitLab으로 미러링하는 작업이 추가되었는데, 이참에 여러 문제점들을 해결하고자 GitActions를 통해 배포를 진행해보도록 하겠다.

GitActions에서 CI/CD를 진행하면 위에서 말한 노출되면 안되는 설정 정보들을 Git submodule을 적용해서 관리하도록 해보겠다.
...
[EC2 생성 및 프로젝트 연결](https://percyfrank.github.io/docker/docker01/)
...
![image](https://user-images.githubusercontent.com/85394884/216414798-ae4a4f47-a939-4d37-a074-b54757239439.png)
...
![image](https://user-images.githubusercontent.com/85394884/216424657-97f37b5a-f0e6-47f6-998d-7d9d71605e31.png)

![image](https://user-images.githubusercontent.com/85394884/216424880-0f15ff46-05a0-4e15-be1c-4cb621d8e4b7.png)
...
![image](https://user-images.githubusercontent.com/85394884/216430460-f01e5a89-29b2-4c58-b203-24a7cfeed28a.png)

`.gitmodules` 파일을 간단히 살펴보면, submodule repo의 경로와 url을 알려준다.
```
[submodule "gitaction_cicd_tes2_config"]
	path = gitaction_cicd_tes2_config
	url = https://github.com/Likelion-Team6-test/gitaction_cicd_tes2_config.git
```
...
task copyPrivate(type: Copy) {
	copy {
		from './gitaction_cicd_tes2_config '
...
즉, docker compose는 컨테이너로 올릴 image들을 docker hub로부터 pull받아서 한꺼번에 올려주는 작업을 한다.
...
services:
  web:
    container_name: team6-test
    image: percyfrank/teamproject
    ports:
      - '8080:8080'
```
...
나의 경우 id는 percyfrank, image 저장소 repo 이름은 아래 사진과 같이 teamproject로 했다.

![image](https://user-images.githubusercontent.com/85394884/216513741-040aed0e-7007-4a3d-a73f-9fda834bf860.png)
...
다음으로, GitActions에서 CI/CD 하기 위해 Docker image 생성을 위한 DockerFile 작성을 해보겠다.
...
### 5. GitActions CI/CD 스크립트
---

프로젝트 root 경로에 `./github/workflows` 디렉토리를 만들고 `cicd.yml` 파일을 생성한다.

![image](https://user-images.githubusercontent.com/85394884/216554202-3be94a09-0cdb-4586-84bc-aedf9287dd4b.png)

여기서부턴, 스크립트의 내용을 순서대로 간략히 설명하겠다.

#### 5-1. `on`

main 브랜치에 push 혹은 pull request가 일어나면 해당 event가 실행된다.
...
GitActions의 workflow는 다양한 job으로 구성되며 job은 steps로 구성이 된다.
...
다음으로, GitActions에서 사용될 JDK를 세팅하는 부분이다.
...
나의 경우 `image: percyfrank/teamproject` 였기 때문에 `teamproject` 를 넣어주었다.
...
- name: EC2 Docker Run
  ...
  env:
    APP: "teamproject"
    COMPOSE: "/home/ubuntu/compose/docker-compose1.yml"
  with:
    username: ubuntu
...
✅ 추가적으로, gradle build 시 springboot 2.5 버전 이후로는 jar 파일이 2개가 생겨 GitActions 작업이 제대로 작동하지 않는다.
...
코드를 push 후, GitActions 작업이 성공하면 `docker ps -a`로 컨테이너가 올라갔는지 확인한다.

![image](https://user-images.githubusercontent.com/85394884/216573780-1db3c568-90e8-4b36-96f2-e3ae4c4e8004.png)
